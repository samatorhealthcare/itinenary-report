    ...
    <section id="section-content">
        <div class="container my-5" style="max-width: 720px;">
            <div class="card">
                <div class="card-body">
                    <?php if(isset($user)) {?>
                        <h2>Perbaharui Data <?= $user['username']; ?></h2>
                    ...
                    <?php } ?>
                        ...
                        <div class="row">
                            ...
                            <?php } else { ?>
                                <div class="col-12 mb-3">
                                    <label for="input-username" class="form-label mb-0 fw-bold">Username</label>
                                    <input type="text" class="form-control" id="input-username" value="<?= isset($user) ? $user['username'] : ''; ?>" disabled required>
                                    <?php if(isset($_SESSION['ERROR']['input-username'])) { ?>
                                        <div class="alert alert-danger mb-0 py-2 fade show" role="alert">
                                            <?= $_SESSION['ERROR']['input-username'] ?>
                                        </div>
                            ...
                            <div class="col-12 col-md-5">
                                <label for="select-lapor-ke-role" class="form-label mb-0 fw-bold">Lapor Ke Role</label>
                                <select class="form-select" id="select-lapor-ke-role" required>
                                    <?php if(isset($user) && isset($user['reports_to_role'])) { ?>
                                        <option value="">-</option>
                                        <option value="manager" <?= $user['reports_to_role'] == 'manager' ? 'selected' : ''; ?>>Manager</option>
                                        <option value="gmanager" <?= $user['reports_to_role'] == 'gmanager' ? 'selected' : ''; ?>>General Manager</option>
                                        <option value="director" <?= $user['reports_to_role'] == 'director' ? 'selected' : ''; ?>>Direktur</option>
                                    <?php } else { ?>
                            ...
                            <div class="col-12 col-md-7">
                                <label for="select-lapor-ke" class="form-label mb-0 fw-bold">Lapor Ke</label>
                                <select name="input-reports-to" class="form-select" id="select-lapor-ke" required>
                                    <?php if(!isset($user) || $user['reports_to'] == '0') { ?>
                                        <option value="" hidden selected disabled>-</option>
                                    <?php } else { ?>
                                        <option value="" hidden disabled>-</option>
                                    <?php } ?>
                                    <?php foreach($users as $temp) { ?>
                                        <?php if(isset($user) && $user['reports_to'] == $temp['id']) { ?>
                                            <option value="<?= $temp['id']; ?>" class="<?="option-lapor-ke role-" . $temp['role'];?>" style="display: none;" selected><?= $temp['username']; ?></option>
                                        <?php } else { ?>
